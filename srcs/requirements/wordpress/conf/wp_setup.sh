if ! wp-cli.phar --path=/usr/share/wordpress core is-installed 2> /dev/null; then
	wp-cli.phar --path=/usr/share/wordpress core download 2> /dev/null
	...
	wp-cli.phar --path=/usr/share/wordpress core install --url=https://mde-arpe.42.fr:5000 --title=miaumiau --admin_user="$WP_ADMIN_USER" --admin_password="$WP_ADMIN_PASSWORD" --admin_email="$WP_ADMIN_EMAIL"
